...
def student_submit(drv, assignment, sid, points, comments):
    ...
    drv.get('https://canvas.wisc.edu/courses/65854/gradebook/speed_grader?assignment_id={0}#%7B%22student_id%22%3A%22{1}%22%7D'.format(assignment, sid))
    ...
    if DELETE_COMMENT:
        for deletes in comment_delete:
            res = deletes.get_attribute('innerHTML')
            print res
            if comments in res:
                deletes.click()
...
if len(sys.argv) <= 1:
    print 'please give me the file name(without .xlsx): '
    fileName = raw_input()
else:
    fileName = sys.argv[1]
print 'You are now submitting assignment id = ' + str(assignment) + ' test rubrics number:' + str(TESTS_NUM) + ' filename:' + fileName + '.xlsx'
y = raw_input('confirm? \nPlease enter \'y\' to confirm your submission. \nDon\'t do anything when submitting: ')
if y != 'y':
    exit()

driver = webdriver.Chrome()
driver.get("http://canvas.wisc.edu")
...
DELETE_ASSIGNMENT_SELECTOR = '.delete_comment_link.icon-x'
first_rubric = 1
DELETE_COMMENT = 1
