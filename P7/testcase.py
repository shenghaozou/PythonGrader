...
rules = {'and': 'Pass All Tests to Get Points',
         ...
         'select':'Different Siutation I\'ll Give Different Points'}
types = {'script': 'Check script of function',
         'func': 'Test Function',
         'all':'Test a Group of Function'}
...
def readFile(fileName):
    fr = open(TESTER + '/' + fileName, 'r')
    strs = fr.read()
    fr.close()
    return strs

def funcRet(j, test, subTest):
    if subTest:
       print ' \033[94mSubTest of Test ' + str(j) + ': ' + types[test['type']] + ' - ' + checks[test['check']] + '\033[0m'
    else:
    ...
    if test['type'] == 'func':
        ...
        if 'input_file' in funcTest:
            ...
            print ' |- file content:'
            print readFile(funcTest['input_file'])
        ...
        if 'stdout_pat_file' in funcTest and test['check'] == 'stdout_pat':
            ...
            print ' |- file content:'
            print readFile(funcTest['stdout_pat_file'])
        print '  '
    if test['type'] == 'script':
        ...
        print '  '
    if test['type'] == 'all':
        print 'Test Subtype:', types[test['subtype']]
        subtest = test.copy()
        subtest['type'] = test['subtype']
        funcRet(j, subtest, True)
...
for key in gradeSettings.GRADING_RULES_ORDER:
    ...
    if dicts['rules'] == 'groupadd':
        ...
        for point, tests in dicts['groups']:
            ...
            for test in tests:
                z += 1
                funcRet(z, test, False)
            print ''
    else:
        j = 0
        for test in dicts['test']:
            j += 1
            funcRet(j,test, False)
